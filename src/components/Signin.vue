<template>
    <div class="account-pages mt-5 mb-5">
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-md-8 col-lg-6 col-xl-5">

            <div class="card" style="border-radius: 20px; background-color: #2c3e50">

              <div class="text-center account-logo-box">
                <p style="padding-top:15px; font-family:monospace; font-size: 20px; font-weight: bold; color:darkgray">Sign In</p>
              </div>

              <div v-if="error" class="alert alert-danger" style="margin: 15px">
                <button type="button" aria-hidden="true" class="close">×</button>
                <span style="color: red">
                            <b> Failed - </b> {{errorMessage}} </span>
              </div>

              <div class="card-body">
                <div class="card-header">
                  <div style="padding:10px; color: darkgray; font-size:12px " v-if="loading">
                    Loading...
                  </div>

                  <form @submit.prevent="submitLogin">

                    <div class="form-group">
                      <label for="email" style="color:darkgray; font-size:12px; float: left">Email Address</label>
                      <input class="form-control" type="text" name="email" required="" v-model="email" placeholder="Email">
                    </div>

                    <div class="form-group">
                      <label for="password" style="color:darkgray; font-size:12px; float: left">Password</label>
                      <input class="form-control" type="password" required="" v-model="password" name="password" placeholder="Password">
                      <router-link to="/forgot"><a style="font-size: 12px; float: right; color:rgb(255,0,127)" href="" class="text-muted forgot"><i style="color:rgb(255,0,127)" class="fa fa-lock mr-1"></i> Forgot your password?</a></router-link>
                      <br>
                    </div>



                    <div class="form-group text-center mt-2" style="width: 100px;">
                        <button :disabled="loading" class="btn btn-bordered btn-danger waves-effect waves-light" style="width: 361px" type="submit">Log In</button>

                    </div>

                    <p class="hr-text line" style="color: darkgray; margin-left: auto; margin-right: auto; text-align: center" data-content="OR">OR</p>

                    <div class="form-group text-center mt-2" style="width: 100px;">

                      <button :disabled="loading" class="btn btn-bordered waves-effect waves-light" style="width: 361px" type="submit"><input style="width: 30px; height: 30px; float:left; border-radius: 20px" type="image" src="data:image/png;base64," />Sign in with Google</button>

                    </div>

                  </form>
                  <!--                            <div class="col-12">-->
                  <!--                                  <button @click="faceBook" :disabled="loading" class="btn width-md btn-bordered btn-danger waves-effect waves-light" type="submit">Facebook</button>-->

                  <!--                            </div>-->

                </div>
                <!-- end card-body -->
              </div>
              <!-- end card -->

              <div class="row">
                <div class="col-sm-12 text-center">
                  <p class="text-muted">Don't have an account? <router-link to="Register"  class="text-primary ml-1"><b>Sign Up</b></router-link></p>
                </div>
              </div>

            </div>
            <!-- end col -->
          </div>
          <!-- end row -->
        </div>
        <!-- end container -->
      </div>
    </div>
</template>

<script>
import store from '../store/index'
import axios from 'axios'
export default {
  name: 'Signin',
  components: {

  },
  data(){
    return {
      loading: false,
      email : "",
      password : "",
      error: null,
      errorMessage: null
    }
  },
  methods: {
    submitLogin() {
      this.loading = true
      axios.post('auth/login', {
        email: this.email,
        password: this.password
      }).then(response => {
        this.loading = false
        // login user, store the token and redirect to dashboard
        store.commit('loginUser')
        store.commit('token',response.data.data.token )
        localStorage.setItem('token', response.data.data.token)
        console.log(response.data.data.token)
        this.$router.push({ name: 'Home' })
      }).catch(error => {
        this.loading = false
        this.error = true
        this.errorMessage = error.message
        if(this.errorMessage === null){
          this.errorMessage = "Turn on Database"
        }else{
          this.errorMessage = error.response.data.error.message
        }
      });
    },
  }
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
h3 {
  margin: 40px 0 0;
}
ul {
  list-style-type: none;
  padding: 0;
}
li {
  display: inline-block;
  margin: 0 10px;
}
a {
  color: #42b983;
}
h4:before, h4:after {
  content: "";
  flex: 1 1;
  border-bottom: 2px solid #000;
  margin: auto;
}
</style>
